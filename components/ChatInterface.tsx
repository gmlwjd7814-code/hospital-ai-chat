...
export default function ChatInterface() {
  ...
  return (
    <div className="flex flex-col h-screen bg-pink-50">
      {/* 헤더 */}
      <header className="bg-white border-b border-pink-100 px-4 py-3 flex items-center gap-3 shadow-sm">
        ...
        <div className="ml-auto">
          <span className="text-xs text-pink-400 bg-pink-50 px-2 py-1 rounded-full border border-pink-100">
            참고용 상담 서비스
          </span>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4 gap-4">
            <div className="w-20 h-20 rounded-full bg-pink-100 border-4 border-pink-200 flex items-center justify-center text-4xl">
              🏥
            </div>
            <div>
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-2 max-w-sm">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-sm px-3 py-2 bg-white border border-pink-200 text-pink-600 rounded-full hover:bg-pink-50 transition-colors shadow-sm"
                >
                  {q}
                </button>
              ))}
        ...
        {loading && (
          <div className="flex justify-start mb-4">
            <div className="w-9 h-9 rounded-full bg-pink-500 flex items-center justify-center text-white text-sm font-bold mr-2 mt-1">
              AI
            </div>
      ...
      {/* 빠른 질문 (대화 중) */}
      {messages.length > 0 && (
        <div className="px-4 py-2 flex gap-2 overflow-x-auto border-t border-pink-100 bg-white scrollbar-hide">
          {QUICK_QUESTIONS.map((q) => (
            <button
              key={q}
              onClick={() => sendMessage(q)}
              disabled={loading}
              className="flex-shrink-0 text-xs px-3 py-1.5 bg-pink-50 border border-pink-200 text-pink-600 rounded-full hover:bg-pink-100 hover:border-pink-300 hover:text-pink-700 transition-colors disabled:opacity-50"
            >
              {q}
            </button>
          ))}
      ...
      <div className="bg-white border-t border-pink-100 px-4 py-3">
        <div className="flex gap-2 items-end max-w-3xl mx-auto">
          <textarea
            ...
            className="flex-1 resize-none border border-pink-200 rounded-2xl px-4 py-3 text-sm text-black focus:outline-none focus:ring-2 focus:ring-pink-300 focus:border-transparent bg-pink-50 max-h-32 disabled:opacity-50 leading-relaxed"
          ...
          <button
            ...
            className="w-12 h-12 bg-pink-500 text-white rounded-full flex items-center justify-center hover:bg-pink-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0"
          >
